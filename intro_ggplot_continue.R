...
# Ес##################ли мы хотим выразить графиком одновременно данные по полу и по курению, то мы должны задать две разные эстетики
...
########## Более продвинутые методы визуализации #############
...
##### Использование функций семейства stat
...
## Столбчатая диаграмма для двух групп с отложенными стандартными ошибками

fev %>% 
  group_by(Sex) %>%  
  summarize(Mean_Height  = mean(Height), SD = sd(Height), SE = SD/sqrt(n())) %>% 
    ggplot(aes(x = Sex, y = Mean_Height)) +
      geom_col() +
      geom_errorbar(aes(ymin = Mean_Height - SE, ymax = Mean_Height + SE), width = 0.2) +
      labs(x = "Пол", y = "Средний рост") 



### Секторные диаграммы для соотошения полов среди курящих и не курящих
...
ggplot(ages, aes(x ="", y = Prop, fill = Sex)) + 
